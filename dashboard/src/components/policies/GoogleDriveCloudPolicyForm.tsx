...
import { GoogleDriveCloudConfig } from '@/types/policy'
import {
  getGoogleDriveConnections,
  getGoogleDriveProtectedFolders,
  initiateGoogleDriveConnection,
  updateGoogleDriveBaseline,
  type GoogleDriveProtectedFolderStatus,
} from '@/lib/api'
...
interface GoogleDriveCloudPolicyFormProps {
  ...
  onChange: (config: GoogleDriveCloudConfig) => void
}
...
export default function GoogleDriveCloudPolicyForm({
  config,
  onChange,
}: GoogleDriveCloudPolicyFormProps) {
  const [connections, setConnections] = useState<Connection[]>([])
  const [loading, setLoading] = useState(true)
  const [connecting, setConnecting] = useState(false)
  const [customPolling, setCustomPolling] = useState(false)
  const [folderStatuses, setFolderStatuses] = useState<Record<string, GoogleDriveProtectedFolderStatus>>({})
  ...
  const loadConnections = async () => {
    try {
      setLoading(true)
      const data = await getGoogleDriveConnections()
      setConnections(data)
      ...
      toast.error('Failed to load Google Drive connections')
    } finally {
      setLoading(false)
    }
  }
  ...
  const loadFolderStatuses = async (connectionId: string) => {
    try {
      ...
      const data: GoogleDriveProtectedFolderStatus[] = await getGoogleDriveProtectedFolders(connectionId)
      const map: Record<string, GoogleDriveProtectedFolderStatus> = {}
      data.forEach((folder) => {
    ...
    try {
      ...
      const payload =
        scope === 'selected'
          ...
          : undefined
      await updateGoogleDriveBaseline(config.connectionId, payload)
  ...
  const updateConfig = (key: keyof GoogleDriveCloudConfig, value: any) => {
    ...
    <div className="space-y-6">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Only Google Drive activity after this timestamp will appear in the dashboard.
                </p>
              </div>
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          How frequently the system checks Google Drive for new activity.
        </p>
      </div>
