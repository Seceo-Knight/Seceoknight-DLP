...
- **PDF content extraction** — added same day. `pdftotext` (poppler-utils, also auto-installed by `install-agent.ps1`) reads a PDF's embedded text layer directly when one exists; if a PDF has no meaningful text layer (a scan or photographed document), it automatically falls back to rasterizing up to 10 pages with `pdftoppm` and OCR'ing each with Tesseract. Covers file-write and USB-transfer monitoring — a confidential contract saved or copied to USB as a scanned PDF is no longer invisible to the classifier. *Not yet compiled/tested on a real Windows machine — verify before production use.*
...
The main remaining gap is concentrated in two areas: **identity federation** (no enterprise SSO/SAML/LDAP — MFA is already solved) and **high availability** (single-node event store, single manager instance). Fixing the two P0 items (SSO + OpenSearch HA) would make this enterprise-deployable for organizations without strict regulated-industry requirements. Completing the P1 items (RS256, HA manager, chunk-based fingerprinting, and verifying the new agent OCR/PDF code on real Windows hardware) would bring it to the level of mid-market commercial DLP (Forcepoint, Digital Guardian). Email DLP content inspection, content-level browser extension coverage, and macOS support are what remain to separate Symantec/Purview-tier from mid-market.
